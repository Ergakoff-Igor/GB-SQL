...
CREATE OR REPLACE VIEW Mas_1 AS
SELECT body, to_user_id, from_user_id FROM messages
WHERE to_user_id = 1 OR from_user_id = 1;

SELECT * FROM Mas_1;

# найдите друзей пользователя с id = 1 и поместите выборку в представление; (решение задачи с помощью CTE)
CREATE VIEW Friends_1 AS
WITH
  cte1 AS (SELECT initiator_user_id FROM friend_requests WHERE target_user_id = 1 AND status = 'approved'),
  cte2 AS (SELECT target_user_id FROM friend_requests WHERE initiator_user_id = 1 AND status = 'approved')
SELECT *
FROM cte1
...
# Найти друзей друзей 1
WITH
  cte1 AS (SELECT initiator_user_id FROM friend_requests WHERE target_user_id IN (SELECT initiator_user_id FROM Friends_1) AND status = 'approved' AND initiator_user_id != 1),
  cte2 AS (SELECT target_user_id FROM friend_requests WHERE initiator_user_id IN (SELECT initiator_user_id FROM Friends_1) AND status = 'approved'AND target_user_id != 1)
SELECT *
FROM cte1
UNION SELECT * FROM cte2
ORDER BY initiator_user_id;
